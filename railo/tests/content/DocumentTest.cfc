...
component extends="mxunit.framework.TestCase" {
	...
	public void function setUp() {
		...
		variables.template = mock(CreateObject("Template")).getPlaceholders().returns([placeholder1, placeholder2])
	...
	public void function AddRemoveSection_Should_Work() {
		assertTrue(variables.document.getSections().isEmpty(), "before a section is added, getSections() should return an empty struct")

		var section1 = mock(CreateObject("Section"))
		var placeholder1 = mock(CreateObject("Placeholder")).getRef().returns("p1")
		variables.document.addSection(section1, placeholder1)

		var sections = variables.document.getSections()
		...
		var placeholder2 = mock(CreateObject("Placeholder")).getRef().returns("p2")
		variables.document.addSection(section2, placeholder2)
		...
		variables.document.removeSection(placeholder1)
	...
	public void function AddSection_Should_HaveNoEffect_When_PlaceholderFilled() {
		...
		variables.document.addSection(section2, placeholder1)
	...
	public void function AddSection_Should_ThrowNoSuchElementException_When_NotExists() {
		var section1 = mock(CreateObject("Section"))
		var placeholder1 = mock(CreateObject("Placeholder")).getRef().returns("p3")
	...
	public void function UseTemplate_Should_KeepSimilarContentAndRemoveRest() {
		var template1 = variables.document.getTemplate()

		var placeholder1 = mock(CreateObject("Placeholder")).getRef().returns("p1")
		var placeholder2 = mock(CreateObject("Placeholder")).getRef().returns("p2")
		var placeholder3 = mock(CreateObject("Placeholder")).getRef().returns("p3")

		var template2 = mock(CreateObject("Template")).getPlaceholders().returns([placeholder1, placeholder3])
		...
		variables.document.addSection(section2, placeholder3) // This should not throw.
		...
		try {
			variables.document.addSection(section2, placeholder3)
			fail("exception should have been thrown")
		} catch (Any e) {}
		variables.document.addSection(section2, placeholder2)
